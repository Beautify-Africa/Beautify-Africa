...
async function fetchUnsplashImages() {
  ...
  for (let page = 1; page <= 8; page++) {
    const res = await fetch(
      `https://api.unsplash.com/search/photos?query=cosmetics+product+photography+mockup&per_page=30&page=${page}&client_id=${ACCESS_KEY}`
    );
...
async function generate() {
  ...
  let currentId =
    existingProducts.length > 0 ? Math.max(...existingProducts.map((p) => p.id)) + 1 : 1;
  ...
  console.log(
    `Successfully generated and wrote ${targetCount} perfectly unique items to seedProducts.js`
  );
}
