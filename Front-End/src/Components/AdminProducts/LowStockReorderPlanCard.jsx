  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-600">
            Recommendations blend low stock with recent sales so the team can reorder before items
            stall.
          </p>
        </div>
        <button
      ...
      {isLoadingReorderPlan ? (
        ...
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Recommendations</p>
              <p className="mt-1 text-2xl font-bold text-stone-900">
                {reorderPlan.summary.recommendationCount}
              </p>
            </div>
            <div className="rounded-lg bg-amber-50 p-3">
              <p className="text-xs uppercase tracking-wide text-amber-700">High priority</p>
              <p className="mt-1 text-2xl font-bold text-amber-900">
                {reorderPlan.summary.highPriorityCount}
              </p>
            </div>
            <div className="rounded-lg bg-emerald-50 p-3">
              <p className="text-xs uppercase tracking-wide text-emerald-700">Lead time</p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-stone-600">
                      SKU: {item.sku || 'N/A'} •{' '}
                      {item.type === 'variant' ? 'Variant' : 'Main stock'}
                    </p>
                    <p className="mt-1 text-xs text-stone-500">{item.note}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs uppercase text-stone-500">Reorder</p>
                    <p className="text-2xl font-bold text-stone-900">{item.recommendedOrderQty}</p>
                    <p className="text-xs text-stone-500 capitalize">{item.urgency}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-lg border border-stone-200 bg-stone-50 px-4 py-6 text-center text-stone-600">
