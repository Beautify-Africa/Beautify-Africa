                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`text-2xl font-bold mt-1 ${stockStatus.color}`}>{item.stock}</p>
                      <span
                        className={`inline-block mt-2 px-2 py-1 rounded text-xs font-bold text-white ${stockStatus.label === 'OUT OF STOCK' ? 'bg-red-600' : stockStatus.label === 'CRITICAL' ? 'bg-red-500' : 'bg-amber-600'}`}
                      >
                        {stockStatus.label}
                      </span>
                    </div>
            ...
            {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-semibold ${pageNum === pagination.page ? 'bg-stone-900 text-white' : 'border border-stone-300 hover:bg-stone-50'}`}
                >
                  {pageNum}
                </button>
              );
