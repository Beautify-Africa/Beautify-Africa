...
export default function useAdminProductsEditor({
  selectedProduct,
  setSelectedProduct,
  loadProducts,
  setSuccessMessage,
  ...
  const saveProduct = useCallback(
    async (event) => {
      ...
      const payload = {
        ...formState,
        images: String(formState.imagesText || '')
          .split(',')
  ...
  const closeAddVariantModal = useCallback(() => {
    setVariantModals((prev) => ({ ...prev, isAddModalOpen: false }));
  }, []);
  const openEditVariantModal = useCallback((variant) => {
    setSelectedVariant(variant);
    setVariantModals((prev) => ({ ...prev, isEditModalOpen: true }));
  }, []);
  const closeEditVariantModal = useCallback(() => {
    setVariantModals((prev) => ({ ...prev, isEditModalOpen: false }));
    setSelectedVariant(null);
  }, []);
  const openStockAdjustmentModal = useCallback((variant) => {
    setSelectedVariant(variant);
