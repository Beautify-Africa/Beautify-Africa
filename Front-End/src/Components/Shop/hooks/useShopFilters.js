...
export function useShopFilters({ shopCatalog, wishlistSet }) {
  ...
  const handleSetSortOption = useCallback((option) => {
    startTransition(() => {
      setSortOption(option);
    });
  }, []);

  const handleSetSelectedBrand = useCallback((brand) => {
    startTransition(() => {
      setSelectedBrand(brand);
    });
  }, []);

  const handleSetSelectedSkinType = useCallback((skinType) => {
    startTransition(() => {
      setSelectedSkinType(skinType);
    });
  }, []);

  const handleSetMaxPrice = useCallback((price) => {
    startTransition(() => {
      setMaxPrice(price);
    });
  }, []);

  const handleSetActiveCollection = useCallback((collection) => {
    startTransition(() => {
  ...
  return {
    ...
    setSelectedSkinType: handleSetSelectedSkinType,
