  ...
  return (
    <section className="rounded-sm border border-stone-200 bg-white px-6 py-10 text-center shadow-sm sm:px-10">
      ...
      <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-stone-600">
        Use the customer account you used during checkout to view purchase history and live shipping
        progress updates.
      </p>
      <Link
        to="/shop?auth=1"
...
export default function TrackOrdersWorkspace() {
  ...
  const { orders, isLoadingOrders, ordersError, lastSyncedAt, refreshOrders } =
    useProfileOrders(token);
  return (
      ...
      <div className="mx-auto max-w-6xl px-6 lg:px-12">
        <header className="mb-9 flex flex-wrap items-end justify-between gap-4 border-b border-stone-200 pb-8">
          <div>
            ...
            <h1 className="mt-2 font-serif text-5xl leading-none text-stone-900 md:text-6xl">
              Track Your Order
            </h1>
            <p className="mt-3 max-w-2xl text-sm text-stone-600">
              Purchase history and shipping stages update as soon as your admin team changes
              shipment status.
            </p>
          </div>
        ...
        {isRestoringSession ? (
          ...
          <section className="rounded-sm border border-stone-200 bg-white px-6 py-12 text-center shadow-sm">
            ...
            <p className="mx-auto mt-3 max-w-lg text-sm text-stone-600">
              Your shipping timeline will appear here once you complete a purchase.
            </p>
            <Link
              to="/shop"
