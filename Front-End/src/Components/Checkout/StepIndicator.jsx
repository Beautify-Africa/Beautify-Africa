        ...
        return (
          <div key={label} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-[9px] uppercase tracking-[0.15em] mt-1 ${isActive ? 'text-stone-900 font-bold' : 'text-stone-400'}`}
              >
                {label}
              </span>
            </div>
            {i < CHECKOUT_COPY.steps.length - 1 && (
              <div
                className={`w-16 h-px mx-2 mb-4 transition-colors duration-500 ${step > num ? 'bg-amber-600' : 'bg-stone-200'}`}
              />
            )}
          </div>
        );
