  ...
  return (
    <article className="overflow-hidden rounded-sm border border-stone-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-stone-100 bg-stone-50 px-6 py-4">
        <OrderStatusBadge
          label={order.isPaid ? 'Payment Cleared' : 'Awaiting Payment'}
          tone={order.isPaid ? 'emerald' : 'amber'}
        />
        <OrderStatusBadge
          label={`Fulfillment: ${STAGE_LABELS[fulfillmentStatus] || 'Processing'}`}
          tone={fulfillmentTone(fulfillmentStatus)}
        />
        <OrderStatusBadge
          label={order.isDelivered ? 'Delivered' : 'In Progress'}
          tone={order.isDelivered ? 'emerald' : 'stone'}
        />
      </div>

      <div className="px-6 py-6">
        <h4 className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-stone-400">
          Packaged Items
        </h4>
